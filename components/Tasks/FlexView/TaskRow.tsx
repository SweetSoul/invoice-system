...
export default function TaskRow({ company_name, limit }: Props) {
	return (
		<div className='flex items-stretch bg-gray-200 rounded-lg my-5'>
			<div className='flex items-center flex-grow-0 flex-shrink-0 basis-auto w-3/12 border-r-2 border-r-gray-300 p-3'>
				<Title as='h3' className='font-medium truncate'>
					{company_name}
				</Title>
			</div>
			<div className='flex items-center flex-grow-0 flex-shrink-0 basis-auto w-auto border-r-2 border-r-gray-300 p-3 hover:bg-gray-400 cursor-pointer'>
				<svg
					stroke='currentColor'
					fill='currentColor'
					strokeWidth='0'
			...
			<div className='flex-grow-0 flex-shrink-0 basis-auto w-8/12'>
				<div className='flex flex-wrap max-w-full max-h-40 overflow-scroll'>
					{tasksData.slice(0, limit ?? tasksData.length).map((t) => (
						<div
							className='flex-grow-0 flex-shring-0 basis-auto w-1/4 cursor-pointer p-2 hover:bg-gray-600 text-center group'
							key={t.id}
						>
